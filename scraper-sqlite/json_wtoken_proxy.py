    ...
    def _request(self):
        ...
        for attempt in range(self._retries + 1):
            ...
            try:
                if self.proxies:
                    try:
                        rr = requests.get("https://httpbin.org/ip", proxies=self.proxies, timeout=10)
                        # rr = requests.get(self.url, proxies=self.proxies, timeout=25)
                        # print(rr.status_code, '-working')
                        print(rr.json(), '-working')
